...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_character_classes_give_options_for_a_character
    ...
    assert_equal __(%w[cat bat rat]), animals.select { |a| a[/[cbr]at/] }
  end
  ...
  def test_scan_is_like_find_all
    assert_equal __(%w[one two three]), 'one two-three'.scan(/\w+/)
  end

  def test_sub_is_like_find_and_replace
    assert_equal __('one t-three'), 'one two-three'.sub(/(t\w*)/) { Regexp.last_match(1)[0, 1] }
  end

  def test_gsub_is_like_find_and_replace_all
    assert_equal __('one t-t'), 'one two-three'.gsub(/(t\w*)/) { Regexp.last_match(1)[0, 1] }
  end
end
